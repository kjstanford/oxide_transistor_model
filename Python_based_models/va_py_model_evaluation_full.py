...
lines = list(fi.input(files = 'evaluate_12_30_v1.lis'))
...
num_devices = len(acq_list[0]) - 1
print(num_devices)
Vg_acq_list = [float(x[0][:-1])*suffix_dict[x[0][-1]] if x[0][-1] in suffix_dict else float(x[0]) for x in acq_list]
Id_acq_lists = [[float(x[y][:-1])*suffix_dict[x[y][-1]] if x[y][-1] in suffix_dict else float(x[y]) for x in acq_list] \
     for y in range(1, num_devices+1)]
print(Vg_acq_list, Id_acq_lists)
...
# ax_Vg_hspice = [np.array(Vg_acq_list) for Id_acq_list in Id_acq_lists]
# ax_Vd_hspice = [ax_Vd_exp[0] for Id_acq_list in Id_acq_lists]
...
c = [yy for xx in ['solid', 'dashdot'] for yy in ['r', 'b', 'g', 'k', 'm']] 
s = [xx for xx in ['solid', 'dashdot'] for yy in ['r', 'b', 'g', 'k', 'm']]
# print(c, s)
