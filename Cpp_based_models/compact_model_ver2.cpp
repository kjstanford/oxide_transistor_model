...
// g++ -std=c++20 compact_model_ver2.cpp -o compact
// compact
...
vector<double> ITO_compact_model(double Vgs, double Vds) {
    ...
    auto f = [=] (double phi){
        return Cox*(Vgs-phiMS-phi) - q*(nfree(Efermi,phi)+ntraps(Efermi,phi)+ndeep(Efermi,phi)-Nch);
    };
    auto df = [=] (double phi){
        return -Cox - q*(dnfree(Efermi,phi)+dntraps(Efermi,phi)+dndeep(Efermi,phi));
    };
    ...
    for (int i = 0; i < N; i++) {
        ...
        double qty = (Vds/N)*(Id_fn(V) + Id_fn(V+Vds/N))/2;
        Id_return += qty; 
    }
